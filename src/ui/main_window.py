...
class MainWindow(ctk.CTk):
    ...
    def _handle_packet(self, packet: dict):
        ...
        if pkt_type == "telemetry":
            ...
            # self.log_panel.log_drone(
            #     f"Alt={parsed.get('altitude', 0):.1f}m  Spd={parsed.get('speed', 0):.1f}m/s  "
            #     f"Bat={parsed.get('battery', -1):.0f}%"
            # )
            self.attitude_graph.update_attitude(roll, pitch, yaw)

        # elif pkt_type == "position":
        #     north = packet.get("north", 0.0)
        #     east = packet.get("east", 0.0)
        #     self.live_map.update_drone_position(north, east)
